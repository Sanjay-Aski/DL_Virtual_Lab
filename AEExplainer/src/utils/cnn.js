...
/**
 * Nearest-neighbor upsample on one matrix.
 *
 * @param {[[number]]} mat Matrix
 * @param {int} factor Upsample ratio on both axes
 */
export const singleUpsample = (mat, factor = 2) => {
  let inputHeight = mat.length;
  let inputWidth = mat[0].length;
  let outputHeight = inputHeight * factor;
  let outputWidth = inputWidth * factor;
  let result = init2DArray(outputHeight, outputWidth, 0);

  for (let r = 0; r < inputHeight; r++) {
    for (let c = 0; c < inputWidth; c++) {
      for (let fr = 0; fr < factor; fr++) {
        for (let fc = 0; fc < factor; fc++) {
          result[r * factor + fr][c * factor + fc] = mat[r][c];
        }
      }
    }
  }
 ...
 * @param {[[number]]} mat Matrix
 */
export const singleSigmoid = (mat) => {
  let height = mat.length;
  let width = mat[0].length;
  let result = init2DArray(height, width, 0);

  for (let r = 0; r < height; r++) {
    for (let c = 0; c < width; c++) {
      result[r][c] = 1 / (1 + Math.exp(-mat[r][c]));
    }
  }
  return result;
}
